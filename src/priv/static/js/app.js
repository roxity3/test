...
require.register("web/static/js/authorized_socket.js", function(exports, require, module) {
"use strict";
...
var authSocket = new _phoenix.Socket("/socket"); //import {Socket} from "deps/phoenix/web/static/js/phoenix";

authSocket.connect();

var token = $('meta[name="guardian_token"]').attr('content');
var channel = authSocket.channel("authorized:lobby", { guardian_token: token });
...
channel.on("shout", function (data) {
  socketTalk(data.body, data.from);
});

var socketTalk = function socketTalk(message, from) {
  var out = '<li>';
  if (from) {
...
var channel_weathering = socket.channel("weathering:lobby", {});
var select_customer = $("#select_customer");
var select_project = $("#select_project");
var select_equipment = $("#select_equipment");
var register_wear = $("#register_wear");
var button_wear = $("#button_wear");
var modal_magnitudes = $("#modal_magnitudes");

console.log("Iniciando button_wear");
button_wear.click(function () {
  // let measurement_id = ""
...
select_customer.change(function () {
  var customer_id = select_customer.find(":selected").val();
...
select_equipment.change(function () {
  var equipment_id = select_equipment.find(":selected").val();
  // console.log("equipment_id:" + equipment_id)
  channel_weathering.push('get_bucket', { equipment_id: equipment_id });
});

channel_weathering.on('populate_projects', function (payload) {
...
channel_weathering.on('populate_equipments', function (payload) {
...
channel_weathering.on('register_wear_parts_bucket', function (payload) {
  register_wear.empty();
...
channel_weathering.on('modal_magnitudes', function (payload) {
  modal_magnitudes.empty();
  modal_magnitudes.append(payload.html);
  // console.log(payload.html)
});

channel_weathering.join().receive("ok", function (resp) {
